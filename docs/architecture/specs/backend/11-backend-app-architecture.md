...
**Вариант упрощения (§2).** Вместо отдельных сборок `DataAccess.Interfaces` и `RabbitMQ.Interfaces` допускается один проект `GM.<Сервис>.Infrastructure.Interfaces`, объединяющий все абстракции инфраструктуры, если команде не нужно разделять границы по технологиям. Для сервисов с PostgreSQL и RabbitMQ каноном остаётся раздельное разбиение из таблицы выше.
...
Внутри сборки `GM.<Сервис>.UseCases` сценарии группируются по **вертикальным срезам** (фича, агрегат или подсистема API).
...
Дерево по одной фиче `<Feature>` (например `Greenhouses`, `Sensors`, `Weather`):
...
**Примеры типов.** `GreenhousesController`, фильтры, привязка моделей к командам/запросам Requestum.
